...
/* -----------------------------------------------------------------------------------------------------------------
	Function pointers to the constructors of the C++ global objects. These are used to initialize global objects before main() is called.
	-	These are defined in the linker script and should be placed in the .init_array section.
	-	They are called by the startup code before main() is executed.
	-	These function pointers are typically used for C++ constructors, but can also be used for C functions if needed.

	Note: The __init_array_start and __init_array_end symbols are defined in the linker script and represent the start and end of the .init_array section.	
																																							*/
...
void _init_CPP()
{
    //for (unsigned long i = 0; i < (unsigned long) (__init_array_end - __init_array_start); i++) 

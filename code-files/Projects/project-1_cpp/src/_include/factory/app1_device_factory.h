#ifndef APP1_DEVICE_FACTORY
#define APP1_DEVICE_FACTORY
...
class App1DeviceFactory : public BaseDeviceFactory 
{
public:
    int num;

    // Ctors/Dtor
    App1DeviceFactory() : num(0) {}
    explicit App1DeviceFactory(bool initialize = false) 
    {
        if( initialize)
            Initialize();
    ...
    {      
        PRINT_DBG_PUBLIC();
        printf("This `App1` will display terminal output if you have installed and configured it\r\n");
        printf("It will all so ask for input (By default disabled in case you have no terminal) \r\n");
